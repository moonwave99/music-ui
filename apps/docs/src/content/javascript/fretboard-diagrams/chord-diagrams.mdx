...
- a `0` stands for the corresponding open string;
- a `x` mutes the corresponding string.
...
<Callout type="info">
  When the [`initChords`](#the-initchords-function) function is used, you can
  directly set `data-display-property="note"` on the element to display the note
  name.
</Callout>
