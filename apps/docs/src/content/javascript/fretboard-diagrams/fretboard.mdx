...
Rendering a fretboard requires a `#fretboard` target element:
...
You can select a different element via the `element` option:

```html
<div class="my-fretboard"></div>
```
...
const fretboard = new Fretboard({
  element: ".my-fretboard",
});
fretboard.render();
```

<Callout type="info">
  All the examples of this page assume a default `#fretboard` element present
  inside the document.
</Callout>
...
<Fretboard display="contain" fretCount={5} width={600} />
...
<Fretboard display="cover" fretCount={5} width={600} />
...
<Callout type="info">
  ...
  [`@music-ui/fretboard` package](/javascript/fretboard-diagrams) contains scale
...
[See the chord diagrams page for more details](/javascript/fretboard-diagrams/chord-diagrams).
...
[See the scale diagrams page for more details](/javascript/fretboard-diagrams/scale-diagrams).

## Highlight areas

You can highlight portions of the fretboard with the [`Fretboard.highlightAreas`](/api/classes/_music-ui_fretboard.Fretboard.html#highlightareas) method:
...
fretboard.render().highlightAreas(
  [
    { string: 6, fret: 1 },
    { string: 1, fret: 3 },
  ],
...
You can clear all the current highlighted areas with the [`Fretboard.clearHighlightedAreas`](/api/classes/_music-ui_fretboard.Fretboard.html#clearhighlightedareas) method.
...
| `highlightAreasPadding` | `number`                                               | 10                                     | The highlight areas padding                                        |
| `highlightAreasRadius`  | `number`                                               | 10                                     | The highlight areas border radius                                  |
| `highlightAreasStroke`  | `string`                                               | `transparent`                          | The highlight areas stroke color                                   |
| `highlightAreasFill`    | `string`                                               | `#ff636c44`                            | The highlight areas fill color                                     |
