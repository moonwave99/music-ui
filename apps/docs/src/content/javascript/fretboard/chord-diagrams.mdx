...
**Note**: the enharmonic equivalents are preferred in altered scenarios like the sharp 9, see:
...
You can display the open string positions by passing `showOpenStrings: true` to the `renderChord` method.  
...
fretboard.renderChord({ input: "x32010", showOpenStrings: true });
```

Output:

<Chord input="x32010" showNoteNames showOpenStrings showName={false} />
...
Need to play that `F7#9` in first position? No worries!
